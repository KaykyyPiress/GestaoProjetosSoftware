...
export default function RelatoOcorrencia({ route }) {
  ...
  async function enviarEmailOcorrencia(
    nomeCidadao,
    emailCidadao,
    tipoOcorrencia
  ) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log('Falha ao enviar e-mail da ocorrência:', msg);
      } else {
        console.log('E-mail da ocorrência enviado com sucesso.');
      }
    } catch (err) {
      console.log('Erro ao enviar e-mail da ocorrência:', err);
    }
  }
  ...
  async function handleEnviar() {
    ...
    // AQUI entra o envio do e-mail
    try {
      if (cpf) {
        const { data: cidadao, error: erroCidadao } = await supabase
          .from('table_cidadao')
          .select('nome_cidadao, email_cidadao')
          .eq('cpf_cidadao', cpf)
          .single();

        if (!erroCidadao && cidadao) {
      ...
      console.log('Erro ao buscar cidadão para e-mail:', err);
    }
  ...
  async function handleAvaliar(nota) {
    ...
    const { error } = await supabase.from('table_avaliacao').insert([
      {
        nota,
        cpf_cidadao: cpf || null,
      },
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
            ...
            {!avaliacaoEnviada ? (
              ...
              <>
                ...
                <Text style={styles.modalSubtitulo}>
                  ...
                  {estrelas === 1 ? 'estrela' : 'estrelas'} foi registrada.
                </Text>
                <TouchableOpacity
      ...
      <Text style={styles.label}>
        Cidade <Text style={styles.obrigatorio}>*</Text>
      </Text>
      <TextInput
      ...
      <Text style={styles.label}>
        Rua <Text style={styles.obrigatorio}>*</Text>
      </Text>
      <TextInput
      ...
      <View style={styles.labelRow}>
        <Text style={styles.label}>
          Descrição <Text style={styles.obrigatorio}>*</Text>
        </Text>
        <Text
          style={[
            styles.contador,
      ...
      <TextInput
        ...
        onChangeText={(t) => {
          setDescricao(t);
          if (t.trim().length >= 80)
            setErros((e) => ({ ...e, descricao: null }));
        }}
        multiline
        numberOfLines={4}
      />
      {erros.descricao && (
        <Text style={styles.erroTexto}>{erros.descricao}</Text>
      )}
